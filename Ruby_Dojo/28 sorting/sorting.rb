...
# global variables                                                                                               #
##################################################################################################################
TITLE=                'sorting'                                                # title for header
...
DRILL_NAME=           '28 sorting'                                             # name of this drill
...
#                                                                                                                #
  ...
  result=0                                                                                                       #
  questions=Array.new(9){|index| index+1}                                                                        #
...
#..............................................................................
  question_string=<<-STRING
write code to sort an array of numbers using the bubble sort method. the bubble
sort starts out setting the state of the program as 'unsorted' then starting at
the beginning of the array it compares the current item with the previous item
and swaps them as needed. this process countinues to the end of the array then
the program checks the state to see if the array has been sorted. if not then
the process starts again. clue: if a swap has been preformed then the array is
not sorted!

use array..................list=15,20,1,5,16,3,7,14,2,12,9,10,4,6,18,13,8,11,19,17]
STRING
  ...
  while swap>0
    ...
    while current_index<list.length
      if current_index>0
        ...
        if list[previous_index]>list[current_index]
          list[previous_index],list[current_index]=list[current_index],list[previous_index]
          swap+=1
        end
      end
      current_index+=1
...
list=[15,20,1,5,16,3,7,14,2,12,9,10,4,6,18,13,8,11,19,17]
...
#..............................................................................
  question_string=<<-STRING
what are the two easiest sorting algorithms?
STRING
  answer="the two easiest sorts are the 'selection' and 'insertion' sorts"
  answers=<<-STRING
the two easiest sorts are the 'selection' and 'insertion' sorts
the two easiest sorts are the 'bubble' and the 'shift' sorts
the two easiest sorts are the 'merge' and the 'heap' sorts
the two easiest sorts are the 'quick' and the 'shell' sorts
STRING
  result,drill_status=multiple_choice(question_string,answer,answers,result,random_question,questions_to_go,drill_status)
end  
...
STRING
  answer='there are many sorting algorithms'
  answers=<<-STRING
there are many sorting algorithms
there are two sorting algorithms
there are at least ten sorting algorithms
there are no more than twenty sorting algorithms
STRING
  result,drill_status=multiple_choice(question_string,answer,answers,result,random_question,questions_to_go,drill_status)
end  
...
#..............................................................................
  question_string=<<-STRING
why is sorting an important operation in ruby programming?
STRING
  answer='it would be helpful for searching'
  answers=<<-STRING
it would be helpful for searching
it would be helpful for organizing data
it would help the efficency of the programming
it would be helpful for program maintenence
STRING
  result,drill_status=multiple_choice(question_string,answer,answers,result,random_question,questions_to_go,drill_status)
end  
...
#..............................................................................
  question_string=<<-STRING
write code to sort an array of numbers using an insertion sort. An insertion
sort starts at index 1. it removes the element then compares the element to
the previous ones inserting the element in the correct location. then the index
is moved forward and the next index is re moved and compared to the previous
items again. this is done until the end of the array is reached where the
array will have been sorted.

use array..............list=[15,20,1,5,16,3,7,14,2,12,9,10,4,6,18,13,8,11,19,17]
STRING
program_string=<<-STRING
def insertion_sort(list)
    for key_pos in (1..list.length-1)
      key_value=list.slice!(key_pos)
      scan_pos=key_pos-1
      while (scan_pos>=0) && (list[scan_pos]>key_value)
        scan_pos-=1
      end
      list.insert(scan_pos+1,key_value)
    end
    return list
end
list=[15,20,1,5,16,3,7,14,2,12,9,10,4,6,18,13,8,11,19,17]
...
#..............................................................................
  question_string=<<-STRING
write code to sort an array of numbers, use a selection sort to do this. A
selection sort starts at the beginning of the array comparing the rest of the
array to the current item then swapping it for the smallest number. This is
done until the end of the array is reached, at that point the array will
beginning sorted.
...
def selection_sort(list)
  ...
  while cur_index<list.length
    scan_index=cur_index+1
    while scan_index<list.length
      if list[cur_index]>list[scan_index]
        list[cur_index],list[scan_index]=list[scan_index],list[cur_index]
      end
      scan_index+=1
    end
    cur_index+=1
...
#..............................................................................
  question_string=<<-STRING
when it comes to swapping values, what is the following program considered as?
...
puts b
STRING
  answer='the best way to swap values'
  answers=<<-STRING
the best way to swap values
the wrong way to swap values
the correct way to swap values
the only way to swap values
STRING
  result,drill_status=multiple_choice(question_string,answer,answers,result,random_question,questions_to_go,drill_status)
end  
...
puts b
STRING
  answer='the correct way to swap values'
  answers=<<-STRING
the correct way to swap values
the wrong way to swap values
the best way to swap values
...
STRING
  answer='the wrong way to swap values'
  answers=<<-STRING
the wrong way to swap values
the correct way to swap values
the best way to swap values
